...
export default function AdminCategoriesPage() {
  ...
  const handleDelete = async (id: number, name: string) => {
    if (!confirm(`Delete category "${name}"? Companies using this category will not be affected.`)) return
    setDeletingId(id)
    setError(null)
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 text-sm mt-1">
          Manage company categories used in the company form.
        </p>
      </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            {loading ? (
              ...
              <ul className="divide-y divide-slate-100">
                {categories.map((cat) => (
                  <li key={cat.id} className="flex items-center gap-3 px-6 py-4 hover:bg-slate-50 transition-colors">
                    {editingId === cat.id ? (
                      /* Edit mode */
