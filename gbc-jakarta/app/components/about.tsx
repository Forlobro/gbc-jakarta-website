...
const GBC_REGIONS = [
  {
    label: "Asia Timur Laut",
    cities: [
      { flag: "🇨🇳", name: "Tiongkok (Shanghai, Shenyang, Guangzhou, Chongqing)" },
      { flag: "🇯🇵", name: "Jepang (Tokyo)" },
    ],
  },
  {
    label: "Asia Tenggara dan Pasifik",
    cities: [
      { flag: "🇲🇾", name: "Malaysia (Kuala Lumpur)" },
      { flag: "🇻🇳", name: "Vietnam (Ho Chi Minh City)" },
      { flag: "🇹🇭", name: "Thailand (Bangkok)" },
      { flag: "🇮🇩", name: "Indonesia (Jakarta)" },
      { flag: "🇵🇭", name: "Filipina (Manila)" },
    ],
  },
  {
    label: "Asia Barat Daya",
    cities: [{ flag: "🇮🇳", name: "India (Mumbai, Bengaluru)" }],
  },
  {
    label: "Asia Tengah",
    cities: [
      { flag: "🇷🇺", name: "Rusia (Moskwa)" },
      { flag: "🇺🇿", name: "Uzbekistan (Tashkent)" },
    ],
  },
  {
    label: "Timur Tengah / Afrika",
    cities: [
      { flag: "🇮🇷", name: "Iran (Teheran)" },
      { flag: "🇰🇪", name: "Kenya (Nairobi)" },
    ],
  },
  {
    label: "Amerika Latin",
    cities: [{ flag: "🇲🇽", name: "Meksiko (Kota Meksiko)" }],
  },
  {
    label: "Amerika Utara",
    cities: [{ flag: "🇺🇸", name: "Amerika Serikat (Los Angeles, New York)" }],
  },
];
          ...
          <div className="rounded-2xl overflow-hidden shadow-xl">
            <div className="relative w-full" style={{ paddingTop: "56.25%" }}>
              <iframe
                ...
                src="https://www.youtube.com/embed/J4aWZjyJ3A4"
        ...
        <ScrollReveal>
          <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-8 md:p-12">
            <div className="text-center mb-10">
              <span className="inline-block bg-primary text-white text-sm font-semibold px-6 py-2.5 rounded-full">
                GBC (Gyeonggi Business Center) memiliki 19 lokasi di 14 negara.
              </span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {GBC_REGIONS.map((region) => (
                <div key={region.label} className="flex flex-col gap-3">
                  <span className="inline-block bg-amber-400 text-white text-[0.7rem] font-bold uppercase tracking-wide px-3 py-1 rounded-md w-fit">
                    {region.label}
                  </span>
                  <div className="flex flex-col gap-2">
                    {region.cities.map((city) => (
                      <div key={city.name} className="flex items-start gap-2">
                        <span className="text-base mt-0.5 shrink-0">{city.flag}</span>
                        <span className="text-sm text-text-light leading-snug">{city.name}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <ScrollReveal className="max-w-4xl mx-auto mb-16">
          <div className="rounded-2xl overflow-hidden shadow-xl">
            <div className="relative w-full" style={{ paddingTop: "56.25%" }}>
              <iframe
                className="absolute inset-0 w-full h-full"
                src="https://www.youtube.com/embed/VtZSCi0TgqI"
